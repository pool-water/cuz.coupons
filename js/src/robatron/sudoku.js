...
(function(root){
    ...
    sudoku.prng = new seedrandom("The sun comes up just about as often as it " +
    ...
    sudoku._rand_range = function(max, min){
        ...
        */
        min = min || 0;
        if(max){
            return Math.floor(sudoku.prng() * (max - min)) + min;
        } else {
    ...
    // Set Seed
    // -------------------------------------------------------------------------
    sudoku.set_seed = function(seed){
        /* Set the seed for random number generation. This makes puzzle
         * deterministic with a seed.
        */
      sudoku.prng = new seedrandom(seed);
    };
